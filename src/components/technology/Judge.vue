...
<template >
    <div class="Judge">
		<!-- 导航 --> 
        <Header title='判断题库'></Header> 
		<div @click="goBackInfo" style="width: 50px;height: 50px;position:relative;margin-top:-50px;">
		<!-- 做组件返回处理 -->
		</div>		
		<div class="headBox">
			<h3><span class="addTi" @click="openAddInfo" v-if="hasQx('teach:quest:addKind')">+</span>每道判断题分数为<input v-model="scores" v-on:keyup="scoreFun" @keyup=keyUpFun />分</h3>
		</div>
		<div class="group-div">
        ...
        <el-dialog title="判断题" :visible.sync="showAdd">
            <el-form :model="form">
				...
				<div>	
					<p style="font-size:18px;">
						...
						<span>选项（勾选<b style="font-size:30px;font-weight:300">o</b>默认该选项为正确答案）</span>
					</p>
            ...
            <div slot="footer" class="dialog-footer">
				...
				<el-button @click="showAdd = false" style="display: inline;width:45%">取 消</el-button>
            </div>
        </el-dialog>
...
export default {
	...
	methods: {
		...
		// 		return this.resData	
		// 	}else{
		// 		return resData.filter(function(info){
		...
		async AddInfo(){
			let ywkData ={
				accessToken:'',
				// subjectId:this.objJud.subId,
				questKind:this.questKind,//定2判断
				questDesc:'',
				questAnswer:'',
				// level:this.objJud.levelId,
				// score:this.scores,
				itemList:[]
			},cflag,b=[]
		...
		scoreFun(){
			...
			allScore = this.objJud.scoresAll
			...
			if(res.code===1100){
				...
				this.questList.forEach(function(item,index){
					...
					if(answers.length>0){
						...
						itemobj.arr=[]
						itemobj.answers=Number(answers)
				...
				if(this.objJud.parentData.length>0){
					let parentData = this.objJud.parentData
					//给输入框的分数赋值
					this.scores = this.objJud.scores
					this.resData.forEach(function(v,i){
