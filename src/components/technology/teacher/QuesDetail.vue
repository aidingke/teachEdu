...
<template>
    <div class="TeachPaperDetail">
		...
		<div class="showPaper">
			...
			<div class="selectBox" style="margin-bottom:5%;border-bottom:none">
				<el-row>
					<el-button type="primary" @click="submitInfo">添加到我的试卷</el-button>
				</el-row>	
			</div>
			<div class="scoreAll" style="display:none">
				<p>
...
export default {
	...
	methods: {
		goBackInfo(){
			let c={}
			c.showPaperShow=false,c.addPaperShow=true,c.paperNumber=5,c.qbId=this.objQues.qbId
		...
		async submitInfo(){
			...
			BigObj.sinData = this.sinData
			BigObj.mulData = this.mulData
			BigObj.JudData = this.JudData
			BigObj.comData = this.comData
			BigObj.DisData = this.DisData
			
			BigObj.scoreA0 = this.scoreA0
			BigObj.scoreA1 = this.scoreA1
			BigObj.scoreA2 = this.scoreA2
			BigObj.scoreA3 = this.scoreA3
			BigObj.scoreA4 = this.scoreA4
			
			BigObj.showPaperShow=true,BigObj.addPaperShow=false
			this.$emit("BigObjInfo",BigObj)
			...
			this.mulData.forEach(function(vv,i){
				...
				vv.arr.forEach(function(v,i){
					if(answers[i].length>0){
						...
						answers[i]=='true'?v.answersl=true:v.answersl=false
