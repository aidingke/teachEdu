...
<template >
    <div class="Single">
		...
		<div class="headBox">
			<h3><span class="addTi" @click="openAddInfo" v-if="hasQx('teach:quest:addKind')">+</span>每道单选题分数为<input v-model="scores" v-on:keyup="scoreFun" placeholder="" />分</h3>
		</div>
		<div class="group-div">
...
<!-- {{resData}} -->
        ...
        <el-dialog title="单选题" :visible.sync="showAdd">
            <el-form :model="form">
...
export default {
	...
	methods: {
		...
		async AddInfo(){
			let ywkData ={
				accessToken:'',
				// subjectId:this.objSin.subId,
				questKind:this.questKind,//定0单选
				questDesc:'',
				questAnswer:'',
				// level:this.objSin.levelId,
				// score:this.scores,//分数不要了
				itemList:[]
			},cflag,b=[]
			...
			if(this.form.name.length<5||this.form.name.length>37)
			{
				...
				//console.log(ywkData);return false
				//更新和添加数据接口在此判断
				if(this.editBtn!=-1){	
		...
		async deleAnswers(index,questId){
			// https://www.mhace.com/edu/teach/quest/delete
		...
		async getList(){//获取单选题列表
			let params = {questKind:this.questKind},newList=[],answers=[],checkAll = this.checkAll
			...
			if(res.code===1100){
				...
				this.questList.forEach(function(item,index){
					...
					if(answers.length>0){
						...
						itemobj.checkedSingles = checkAll
		...
		carculate(x){
			return false
			console.log(x)
			let sc=Number(this.scores),checkS=0
			this.resData.forEach(function(v,i){
				if(v.checkedSingles===true){
					checkS+=sc
				}
			...
			// console.log('不执行累加')
				this.allScores = this.objSin.scoresAll
			}else if(x==1||x==undefined){ //执行累加
			console.log('执行累加')
				this.allScores = this.objSin.scoresAll+checkS
			}
			this.checkScore = checkS
