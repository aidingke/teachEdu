<template>
  ...
  <!--上传文件-->
    <div style="margin-top: 40px">
        ...
        <iframe v-if="showPdf" id='previewPdf' :src="'/static/pdf/web/viewer.html?file=' + fileUrl" height="560" width="100%">
        </iframe>
        
        <span @click="OpenPPt()">打开ppt</span>
        <iframe v-if="pptData" :src="pptData" width='100%' height='100%' frameborder='1'>
        </iframe>

        <span @click="OpenPDF()">打开pdf</span>
        <iframe v-if="previewUrl" :src="previewUrl" type="application/pdf" width="100%" height="100%">
        </iframe>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      OpenPPt(){
          this.pptData = 'https://view.officeapps.live.com/op/view.aspx?src=http://storage.xuetangx.com/public_assets/xuetangx/PDF/1.xls'
      },
      OpenPDF(){
          this.previewUrl = "http://storage.xuetangx.com/public_assets/xuetangx/PDF/PlayerAPI_v1.0.6.pdf"
      },
      check () {
...
//          let url = '/static/pdf/web/demo.pdf'
    ...
    checkSuccess () {
         // 后台返回流的形式，也是我本人项目的使用
      let url = 'https://dluat.hscf.com/api/esm/v1/contractTemplates/load/c08def54aa40412b8b511406fc0271d2/0?access_token=b6cce0c8428c55531d206b4f008fe44e&name=cehsi.pdf'
